...
const SearchedItems=()=>{
    ...
    const {productsData,elecProductsData,clothProductsData,cartProductsData,loading,mar,cartLoad,searchedProductsData,sLoad}=useSelector((state)=>state.productsinfo.data)
    const clsName=()=>{
        if(mar) {
            return 'n'
        } else return 'm'
    ...
    return(
        <div style={{marginTop: '100px'}}>
            {
                sLoad ? <><h1 style={{textAlign: 'center',color: 'red',marginTop: '300px'}}>Product unavailable!</h1></> : <>
                {
                    searchedProductsData.length===0 ? <><h1 style={{textAlign: 'center',color: 'red',marginTop: '300px'}}>Product unavailable!</h1></> : 
                    <>
                    <div className={`row ${clsName()}`}>
                    {
                    searchedProductsData.map((item)=>(
